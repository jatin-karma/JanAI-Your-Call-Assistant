# JanAI - Technical Design Document
...
JanAI is a voice-first AI platform that provides multilingual access to critical information and services via basic phone calls, targeting 500M+ Indians without smartphone access.
...
```python
SYSTEM_PROMPT = """
You are JanAI, a helpful AI assistant providing information to 
...
<Response>
    <Gather input="dtmf speech" action="/gather-language" timeout="5">
        <Say voice="Polly.Aditi" language="hi-IN">
            JanAI में आपका स्वागत है। अपनी भाषा चुनें।
        </Say>
    </Gather>
</Response>
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::janai-audio-prod/*",
      "Condition": {
        "StringEquals": {
...
```
Custom Metrics:
- janai.call.duration (seconds)
- janai.call.cost (rupees)
- janai.stt.confidence (0-1)
- janai.stt.latency (ms)
- janai.ai.latency (ms)
- janai.tts.latency (ms)
- janai.cache.hit_rate (%)
- janai.error.rate (%)
    ...
    steps:
      - name: Deploy to production
        run: |
          aws cloudformation deploy \
            --template-file infrastructure/template.yaml \
            --stack-name janai-prod \
            --parameter-overrides Environment=production
      
      - name: Health check
        run: curl https://api.janai.in/health
```
...
Description: JanAI Infrastructure
...
Resources:
  ...
  CallHandlerFunction:
    ...
    Properties:
      FunctionName: !Sub janai-call-handler-${Environment}
      Runtime: python3.11
      Handler: index.handler
      Code:
        S3Bucket: janai-deployments
      ...
      Environment:
        Variables:
          ...
          ANTHROPIC_API_KEY: !Sub '{{resolve:secretsmanager:janai-${Environment}:SecretString:anthropic_key}}'
  
  # DynamoDB Tables
  SessionsTable:
    Type: AWS::DynamoDB::Table
    Properties:
      TableName: !Sub janai-sessions-${Environment}
  ...
  AudioBucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub janai-audio-${Environment}
  ...
  ApiGateway:
    ...
    Properties:
      Name: !Sub janai-api-${Environment}
      ProtocolType: HTTP
...
|---------|------|--------|---------|
| 1.0 | 2026-02-15 | JanAI Team | Initial design document |
