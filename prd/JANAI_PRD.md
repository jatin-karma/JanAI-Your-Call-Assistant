...
**Status: Deferred for hackathon.** Bhashini's API is not yet available under the "" hackathon free tier. Their platform (bhashini.gov.in/ulca) requires a registered organization account and approval pipeline that takes weeks. For the hackathon:
...
**Demo Script:** *"Aaj Varanasi mandi mein gehu ka bhaav kya hai?"* → JanAI fetches live, responds *"आज वाराणसी मंडी में गेहूँ का भाव ₹2,340 प्रति क्विंटल है। अधिकतम ₹2,380 और न्यूनतम ₹2,300 चल रहा है।"*
...
**1. Named identity reinforcement:** JanAI refers to itself by name, never as "the assistant" or "the AI."
...
def send_alert(phone_hash: str, distress_profile: dict):
    ...
    # A JanAI volunteer calls the user the next day
    # "Hum JanAI se bol rahe hain — kal aap ne baat ki thi..."
...
Beyond mental health, JanAI may receive calls describing:
...
- When limit is hit: *"आपका आज का 10 मिनट का फ्री टाइम खत्म हो गया। कल फिर कॉल करें। या JanAI Pro के लिए janai.in पर जाएं।"*
...
Government currently spends ₹800-1,200 per citizen grievance resolved via call centers. JanAI resolves the same query for ₹0.80 (compute + Twilio cost). Pitch this as a 1,000x efficiency gain. Target: state government digital mission (e.g., MahaIT, Tamil Nadu e-Governance) as a bulk subscriber paying per resolved query or per toll-free minute.

**Revenue Stream 2: CSR / Foundation Grants**
Gates Foundation, Tata Trusts, Azim Premji Foundation all fund "last-mile digital access" projects. JanAI's impact metrics (queries answered / correct helpline referrals / distress interventions) are exactly the KPIs these funders track.

**Revenue Stream 3: Affiliate (Done Responsibly)**
NOT "any brand pays for mentions." Instead: JanAI partners only with IRDAI-registered insurance companies and NABARD-approved microfinance institutions. When a user asks about crop insurance, JanAI provides verified information about PM Fasal Bima, then voluntarily mentions: *"एक सरकारी-मान्यता प्राप्त ICICI Lombard का प्लान भी है जिसमें ज़्यादा कवरेज मिलती है — चाहें तो उनका नंबर दूं?"* The referral is transparent, opt-in, and the partner is vetted.

**Revenue Stream 4: Pro Subscriptions (Urban)**
Position for urban users not as "loneliness bot" (too niche) but as:
*"Eyes-Free AI for Busy Professionals"* — driving to work, cooking, managing elderly parents remotely. "Tell me what my mother asked JanAI about her blood pressure medication" becomes a dashboard feature for adult children who've set up JanAI for their parents.
...
4. JanAI responds: Live garlic price from Agmarknet + PM-Kisan eligibility criteria in simple Hindi
5. Follow-up: *"Meri beti ki padhai ke liye koi scholarship hai?"* → JanAI answers from RAG, mentions National Scholarship Portal
...
Users can set:
- Agent name (e.g., "Meena" instead of "JanAI")
...
User says: *"मेरे लिए PM-Kisan register कर दो"*. JanAI collects Aadhaar, land details, bank details over voice, then submits the UMANG API form on their behalf. True agentic workflow. Requires UMANG API access (government partnership needed).
...
DYNAMODB_USERS_TABLE=janai-users          # Phase 2
DYNAMODB_DISTRESS_TABLE=janai-distress    # Phase 2
...
*Document maintained by JanAI team. Last updated: March 2026.*
